/**
 * Default Data-object for a MDC to use as default values when generating an MDC
 * It is important that each key/value pair has a value of either int, string or an array of objects!
 */
const defaultData = {
  date: '241900ZFEB2019',
  missionNumber: 'TR0277',
  callsign: 'VENOM-8',
  ato:
    'VTASK/132vW/20112/241900ZFEB2019//TASKUNIT/ 494th/ICAO:UGKO//AMSNDAT / AI0112 / AI / -/-/2 / -/-/DEPLOC: UGKO / ARRLOC: UGKO// MSNACFT / 4 / F / A - 18C / JEDI51 - 54 / -/-/CORAL9 / YELLOW11//',
  atis: 'UGKO 044500Z 31021KTS 9999M 05/-1 Q1007 NOSIG',
  enr: 'DEP via AKAKI, AAR at SHELL, RTB via OHB',
  etd: '20:00:00Z',
  eta: '+2hrs',
  airfields: [
    {
      name: 'departure',
      id: 1,
      label: 'DEP',
      icao: 'UGKO',
      tcn: '44X',
      gnd: '134.100',
      twr: '134.200',
      elev: '68 ft',
      rwy: '08',
      ils: '109.750',
      lat: 42.177616666,
      lon: 42.48123,
    },
    {
      name: 'recovery',
      id: 1,
      label: 'DEP',
      icao: 'UGKO',
      tcn: '44X',
      gnd: '134.100',
      twr: '134.200',
      elev: '68 ft',
      rwy: '08',
      ils: '109.750',
      lat: 42.177616666,
      lon: 42.48123,
    },
    {
      name: 'alternate',
      id: 1,
      label: 'ALTN',
      icao: 'UGKO',
      tcn: '44X',
      gnd: '132.100',
      twr: '132.200',
      elev: '88 ft',
      rwy: '09',
      ils: '108.900',
      lat: 42.177616666,
      lon: 42.48123,
    },
  ],
  aircraft: {
    id: 1,
    type: 'A-10C',
    nato: 'Warthog',
    role: 'cas',
  },
  element: [
    {
      pilot: 'DEX',
      tcn: '20X',
      laser: '1680',
      mode: '4231',
    },
    {
      pilot: 'ENTROPY',
      tcn: '83X',
      laser: '1681',
      mode: '4232',
    },
    {
      pilot: ' - ',
      tcn: '83X',
      laser: '1682',
      mode: '4233',
    },
    {
      pilot: ' - ',
      tcn: '83X',
      laser: '1683',
      mode: '4234',
    },
  ],
  navSequences: [{ sequence: '0-1-2-3-4-5-6' }, { sequence: '' }, { sequence: '' }],
  navPoints: [],
  radioPresets: [
    { label: 'INT PRI', preset: 'PRI6' },
    { label: 'INT AUX', preset: ' - ' },
    { label: 'GND', preset: 'AUX1' },
    { label: 'TWR:', preset: 'AUX2' },
    { label: 'CTRL', preset: 'AUX3' },
    { label: 'RANGE', preset: ' - ' },
    { label: 'TANK 1', preset: 'PRI13' },
    { label: 'TANK 2', preset: 'PRI14' },
    { label: 'DSTAR', preset: 'AUX6' },
    { label: 'FPONT', preset: 'AUX9' },
    { label: 'VDOO', preset: 'AUX10' },
    { label: 'CSAR', preset: 'AUX18' },
    { label: 'CAS1', preset: 'AUX14' },
    { label: 'CAS2', preset: 'AUX15' },
    { label: 'REPORT', preset: ' - ' },
    { label: 'SNAKE', preset: '230.750' },
  ],
  packageName: 'Alpha',
  packageMembers: [
    {
      callsign: 'COWBOY-6',
      numberOfAC: 3,
      ACtype: 'F/A-18C',
      uhf: 'CORAL11',
      vhf: ' - ',
      tcn: '20X/83X',
      tasking: 'SEAD',
    },
    {
      callsign: '',
      numberOfAC: 0,
      ACtype: 'N/A',
      uhf: '000.000',
      vhf: '000.000',
      tcn: '',
      tasking: '',
    },
    {
      callsign: '',
      numberOfAC: 0,
      ACtype: 'N/A',
      uhf: '000.000',
      vhf: '000.000',
      tcn: '',
      tasking: '',
    },
    {
      callsign: '',
      numberOfAC: 0,
      ACtype: 'N/A',
      uhf: '000.000',
      vhf: '000.000',
      tcn: '',
      tasking: '',
    },
  ],
  ta: 7000,
  tl: 180,
};

export default defaultData;
